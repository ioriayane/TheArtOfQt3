...
function Controller()
{
  ...
  installer.setMessageBoxAutomaticAnswer("archiveDownloadError", QMessageBox.Cancel)  // [1]
}

// コンポーネントが追加されたときのシグナルハンドラ
Controller.prototype.componentAdded = function(component)
{
  //必要なコンポーネントに限定する
  if(component.name == "com.vendor.product"){
    //コンポーネントが読み込まれたときのシグナル
    component.loaded.connect(Controller.prototype.ComponentLoaded)
  }
}
